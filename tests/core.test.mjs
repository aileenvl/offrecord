...
import { parseNotes, mergeNotes, markdown, createSession, Chunker } from '../src/core.js';
const segments = [{id:1,text:'We decided to launch on Friday.',start:0,end:8}];
test('notes require existing evidence and valid structure', () => {
  const result = parseNotes('```json\n{"decisions":[{"text":"Launch Friday","evidence":[1]}],"actions":[],"questions":[]}\n```', segments);
  assert.equal(result.decisions.length,1);
  assert.throws(()=>parseNotes('{"decisions":[{"text":"Invented","evidence":[9]}]}',segments), /evidence/);
  assert.throws(()=>parseNotes('not json',segments));
});
test('unknown owner and deadline stay unknown; duplicate updates merge', () => {
  const notes = parseNotes(JSON.stringify({actions:[{text:'Write notes',evidence:[1]}]}),segments);
...
test('chunker preserves every sample, including final partial chunk',()=>{
  ...
  assert.deepEqual(chunks.map(x=>Array.from(x)),[[1,2,3,4],[5,6,7,8]]);
...
test('invalid model types and oversized evidence are rejected',()=>{
  ...
  assert.throws(()=>parseNotes('{"actions":[{"text":"x","evidence":[]}]}',segments));
});
