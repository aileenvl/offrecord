...
context.on("console", (m) => {
  if (m.type() === "error") {
    errors.push(m.text());
    console.log("BROWSER:", m.text().slice(0, 350));
  }
});
...
if (
  ...
  await page
    .locator("#import-text")
    .fill(
      "Maya: We decided to launch the Chrome extension on Friday.\nLuis: I will test audio capture by Thursday.\nMaya: Can the model work offline? We have not tested that yet.",
    );
  await page.locator("#import").click();
  await page.locator("#analyze").click();
  await page.waitForFunction(
    () =>
      document.querySelector("#activity").textContent.includes("Notes updated"),
    null,
    { timeout: 180000 },
  );
  ...
  await writeFile(
    resolve(output, "notes.json"),
    JSON.stringify(result, null, 2),
  );
  await page.screenshot({ path: resolve(output, "notes.png"), fullPage: true });
  console.log("REAL NOTES:", JSON.stringify(result.state.session.notes));
  await context.close();
}
