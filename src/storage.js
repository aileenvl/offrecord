...
async function transaction(mode, action) {
  ...
  return new Promise((resolve,reject) => {
    ...
    tx.onerror=()=>reject(tx.error); tx.onabort=()=>reject(tx.error ?? Error('Storage transaction aborted'));
  });
}
...
export const listSessions = async () => (await transaction('readonly',store=>store.getAll())).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
