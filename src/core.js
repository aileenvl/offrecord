...
export function mergeNotes(previous, next) {
  return Object.fromEntries(
    Object.keys(emptyNotes()).map((key) => {
      const map = new Map();
      for (const n of [...previous[key], ...next[key]]) {
        ...
        map.set(
          ...
          existing
            ? {
                ...n,
                evidence: [...new Set([...existing.evidence, ...n.evidence])],
              }
            : n,
        );
      }
...
export function markdown(s) {
  const lines = [
    ...
    "",
    s.mode === "demo"
      ? "Sample replay — fixture transcript and notes, no AI inference."
      : "Local inference — review notes against the transcript.",
    `Source: ${s.context.domain}`,
    `Created: ${s.createdAt}`,
    "",
  ];
  for (const [key, title] of [
    ...
    lines.push(`## ${title}`);
    for (const n of s.notes[key])
      lines.push(
        `- ${n.text}${key === "actions" ? ` (Owner: ${n.owner ?? "Unknown"}; Due: ${n.due ?? "Unknown"})` : ""} [segments ${n.evidence.join(", ")}]`,
      );
    if (!s.notes[key].length) lines.push("None captured.");
    lines.push("");
  }
  lines.push("## Transcript");
  if (!s.segments.length) lines.push("Transcript not included in this copy.");
  for (const x of s.segments)
    lines.push(`[${timestamp(x.start)}] #${x.id} ${x.text}`);
  if (s.warning) lines.push("", `Capture note: ${s.warning}`);
  return lines.join("\n");
}
export class Chunker {
  constructor(size, lookahead = 0) {
    this.size = size;
    this.lookahead = lookahead;
    this.pending = new Float32Array(size + lookahead);
    this.used = 0;
  }
  push(samples) {
    const chunks = [];
    ...
    while (offset < samples.length) {
      ...
      if (this.used === this.pending.length) {
        chunks.push(this.pending.slice(0, this.size));
        const following = this.pending.subarray(this.size);
        this.pending = new Float32Array(this.size + this.lookahead);
        this.pending.set(following);
        this.used = this.lookahead;
      }
    }
    return chunks;
  }
  flush() {
    ...
    const tail = this.pending.slice(0, this.used);
    this.used = 0;
    return tail;
  }
}
