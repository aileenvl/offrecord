...
class PCMProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage=({data})=>{
      if(data==='flush') { this.stopped=true; if(this.used) this.port.postMessage({audio:this.buffer.slice(0,this.used)}); this.port.postMessage({flushed:true}); }
    };
  }
  process(inputs) {
    ...
    for(let i=0;i<channels[0].length;i++){
      ...
      if(this.used===this.buffer.length){this.port.postMessage({audio:this.buffer},[this.buffer.buffer]);this.buffer=new Float32Array(2048);this.used=0;}
    }
    return true;
  }
}
