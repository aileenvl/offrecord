...
export async function captureTab(streamId, onChunk, onEnded) {
  ...
  let context,
    node,
    stopped = false;
  // Keep one second pending so a tiny final fragment stays attached to context.
  const chunks = new Chunker(16000 * 12, 16000);
  try {
    context = new AudioContext({ sampleRate: 16000 });
    await context.audioWorklet.addModule(
      new URL("./audio-worklet.js", globalThis.location.href),
    );
    ...
    node.port.onmessage = ({ data }) => {
      if (data.audio)
        for (const chunk of chunks.push(data.audio)) onChunk(chunk);
